    ...
    def __init__(
        ...
        if (idxs_path is not None) and (os.path.exists(idxs_path)):
            ...
            if distributed.get_rank() == 0:
                os.makedirs(os.path.dirname(idxs_path), exist_ok=True)
...
class VOCSegmentationIncrementalMemory(BaseDataset):
    def __init__(
        self,
        test=False, val=False, setting='overlap', step=0, classes_idx_new=[], classes_idx_old=[], classes_idx_prev=[], transform=True,
        transform_args={}, masking_value=0, idxs_path=None, idxs_path_aug = None, aug_ratio = 0, memory_label_visible = True
    ):
        if setting not in ['sequential', 'disjoint', 'overlap']:
            raise ValueError('Wrong setting entered! Please use one of sequential, disjoint, overlap')
        ...
        self.classes_idx_old = classes_idx_old
        self.classes_idx_new = classes_idx_new
        self.classes_idx_prev = classes_idx_prev
        self.memory_label_visible = memory_label_visible
        self.idxs_path_aug = idxs_path_aug
        self.aug_ratio = aug_ratio
        if self.memory_label_visible:
            print(f"process memory at {step}, add images from previous step {step-1}, we only keep the labels trained are on prev step: {classes_idx_prev[0]} -> {classes_idx_prev[-1]}")
        else:
            print(f"get memory at {step}, given the full labels from the memory images of step {step-1}, but we will can not see any labels")
        ...
        file_names = memory_list[f"step_{step}"]["memory_candidates"]
        for x in file_names:
            if 'aug' not in self.split:
                _image = self._image_dir / f"{x}.jpg"
                _cat = self._cat_dir / f"{x}.png"
            else:
                _image = Path(x[0]) # self._image_dir / x.split()[0][1:]
                _cat = Path(x[1]) # self._cat_dir / x.split()[1][1:]
        ...
        if idxs_path_aug is not None:
            if type(idxs_path_aug) is not list:
                self.im_ids_aug = []
                self.images_aug = []
                self.categories_aug = []
                with open(idxs_path_aug, "r") as json_file:
                    aug_list = json.load(json_file)
                file_names = aug_list[f"step_{step}"]["memory_candidates"]
                for x in file_names:
                    if 'aug' not in self.split:
                        _image = self._image_dir / f"{x}.jpg"
                        _cat = self._cat_dir / f"{x}.png"
                    ...
                    self.im_ids_aug.append(x)
                    self.images_aug.append(_image)
                    self.categories_aug.append(_cat)
            else:
                self.im_ids_aug = defaultdict(list)
                self.images_aug = defaultdict(list)
                self.categories_aug = defaultdict(list)
                for file_idx, _idxs_path_aug in enumerate(idxs_path_aug):
                    with open(_idxs_path_aug, "r") as json_file:
                        aug_list = json.load(json_file)
                    file_names = aug_list[f"step_{step}"]["memory_candidates"]
                    for x in file_names:
                        if 'aug' not in self.split:
                        ...
                        self.im_ids_aug[file_idx].append(x)
                        self.images_aug[file_idx].append(_image)
                        self.categories_aug[file_idx].append(_cat)

        else:
            assert self.aug_ratio == 0

        assert len(self.images) == len(self.categories)

    def __getitem__(self, index):
        _img, _target = self._make_img_gt_point_pair(index)
        check_unique = np.unique(np.array(_target).ravel())
        sample = {"image": _img, "label": _target}
        ...
        # Target masking
        check_unique_after_transform = np.unique(sample['label'].ravel())
        #### since the labels have been masked when saving to the memory, so we do not need masking here.
        # sample['label'] = self.transform_target_masking(sample['label'])
        sample["image_name"] = str(self.im_ids[index])
        # sample["memory"] = True
        check_unique_after_masking = np.unique(sample['label'].ravel())

        return sample

    def _make_img_gt_point_pair(self, index):
        if random.random() < self.aug_ratio and self.idxs_path_aug is not None:
            if type(self.idxs_path_aug) is not list:
                _img = Image.open(self.images_aug[index]).convert("RGB")
                _target = Image.open(self.categories_aug[index])
            else:
                file_index = random.randint(0, len(self.idxs_path_aug)-1)
                _img = Image.open(self.images_aug[file_index][index]).convert("RGB")
                _target = Image.open(self.categories_aug[file_index][index])
        else:
            _img = Image.open(self.images[index]).convert("RGB")
            _target = Image.open(self.categories[index])
        return _img, _target
    ...
    def transform_target_masking(self, target):
        assert self.train == True   # Memory only for training
        if self.test:
            # Masking future class object
            # MiB: 255 / PLOP: 0
            label_list = list(set(self.classes_idx_old + self.classes_idx_new + [0, 255]))
            ...
            if self.setting in ['disjoint', 'overlap']:
                if self.memory_label_visible:
                    label_list = list(set(self.classes_idx_old + [0, 255])) # mask in [0,|C0:t|,255]
                else:
                    label_list = list(set([0, 255])) # mask in [0,255]
                target_transform = tv.transforms.Lambda(
                    lambda t: t.apply_(lambda x: x if x in label_list else self.masking_value)
                )
                return target_transform(target)
...
class ADESegmentationIncrementalMemory(BaseDataset):
    def __init__(
        self,
        test=False, val=False, setting='overlap', step=0, classes_idx_new=[], classes_idx_old=[], classes_idx_prev=[],
        transform=True, transform_args={}, masking_value=0, idxs_path=None, idxs_path_aug = None, aug_ratio = 0, memory_label_visible = True
    ):
        if setting not in ['sequential', 'disjoint', 'overlap']:
            raise ValueError('Wrong setting entered! Please use one of sequential, disjoint, overlap')
        ...
        file_names = memory_list[f"step_{step}"]["memory_candidates"]
        for x in file_names:
            _image = Path(x[0]) # _image = self._image_dir / f"{x}.jpg"  # .jpg
            _cat = Path(x[1]) # _cat = self._cat_dir / f"{x}.png"  # .png
            assert _image.is_file(), _image
            assert _image.is_file(), _cat
