...
class VOCIncrementalDataLoader():
    def __init__(self, task, train, mem, val, test, num_workers, pin_memory, memory=None):
        self.task = task
        self.train = train
        self.mem = mem
    ...
    def get_memory(self, config, json_file_name, json_file_augX=None, aug_ratio = 0,  concat=True):
        if self.step > 0:
            self.memory = []
            # keep as what we see in previous step training, memory of prev step images only show prev step labels, no more history labels, 
            classes_idx_prev, _ = get_task_labels('voc', self.name, self.step - 1)
            if json_file_augX is None:
                # only memory 
                print(f'only memory')
                self.memory.append(VOCSegmentationIncrementalMemory(
                    setting=self.setting,
                    step=self.step,
                    classes_idx_new=self.classes_idx_new,
                    classes_idx_old=self.classes_idx_old,
                    classes_idx_prev=classes_idx_prev,
                    idxs_path=config.save_dir.parent / f'step_{self.step}' / f'{json_file_name}',
                    # idxs_path_aug=config.save_dir.parent / f'step_{self.step}' / f'{json_file_aug}',
                    # aug_ratio=0,
                    **self.mem['args'],
                ))
            elif config['replay']['MaskGuide']['Replace']['use_Replace']:
                # replace gen(idxs_path_aug) with the memory(idxs_path)
                print(f'memory with aug ratio = {aug_ratio}')
                if type(json_file_augX) is not list:
                    self.memory.append(VOCSegmentationIncrementalMemory(
                            setting=self.setting,
                            step=self.step,
                            classes_idx_new=self.classes_idx_new,
                            ...
                            classes_idx_prev=classes_idx_prev,
                            idxs_path=config.save_dir.parent / f'step_{self.step}' / f'{json_file_name}',
                            idxs_path_aug=config.save_dir.parent / f'step_{self.step}' / f'{json_file_augX}',
                            aug_ratio=aug_ratio,
                            **self.mem['args'],
                    ))
                else:
                    self.memory.append(VOCSegmentationIncrementalMemory(
                            ...
                            classes_idx_prev=classes_idx_prev,
                            idxs_path=config.save_dir.parent / f'step_{self.step}' / f'{json_file_name}',
                            idxs_path_aug=[config.save_dir.parent / f'step_{self.step}' / f'{_json_file_augX}' for _json_file_augX in json_file_augX],
                            aug_ratio=aug_ratio,
                            **self.mem['args'],
                    ))
            else:
                assert config['replay']['MaskGuide']['Combine']['use_Combine'] == True, f'config["replay"]["MaskGuide"]["Combine"]["use_Combine"] should be True but now is {config["replay"]["MaskGuide"]["Combine"]["use_Combine"]}'
                print(f'add 1 memory and {len(json_file_augX)} lengeneration with ')
                self.memory.append(VOCSegmentationIncrementalMemory(
                    setting=self.setting,
                    step=self.step,
                    classes_idx_new=self.classes_idx_new,
                ...
                for json_file_aug in json_file_augX:
                    assert aug_ratio == 1, f'aug_ratio should be 1 but now is {aug_ratio}'
                    self.memory.append(VOCSegmentationIncrementalMemory(
                        setting=self.setting,
                        step=self.step,
                        classes_idx_new=self.classes_idx_new,
...
class ADEIncrementalDataLoader():
    ...
    def get_memory(self, config, json_file_name, json_file_augX=None, aug_ratio = 0, concat=True):
        if self.step > 0:
            self.memory = []
            # keep as what we see in previous step training, memory of prev step images only show prev step labels, no more history labels, 
            classes_idx_prev, _ = get_task_labels('ade', self.name, self.step - 1)
            if json_file_augX is None:
                # only memory 
                print(f'only memory')
                self.memory.append(ADESegmentationIncrementalMemory(
                    setting=self.setting,
                    step=self.step,
                    classes_idx_new=self.classes_idx_new,
                    classes_idx_old=self.classes_idx_old,
