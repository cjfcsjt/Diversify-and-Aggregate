    ...
    def compute_prototypes(self, config):
        ...
        with torch.no_grad():
            for batch_idx, data in enumerate(self.train_loader):
                ...
                logit, features = self.model(data['image'].cuda(), ret_intermediate=True)
                ...
                norm = torch.norm(features['features'][-1], p=2, dim=1)
                for cls in small_label.unique():
                    if cls in [0, 255]:
                        continue
                    if int(cls) - n_old_classes - 1 < 0:
                        continue
...
def label_to_one_hot(label, logit, n_old_classes, ignore_index=255):
    ...
    for cls_idx in label.unique():
        if cls_idx in [0, ignore_index]:
            continue
        if int(cls_idx) - n_old_classes < 0:
            continue
