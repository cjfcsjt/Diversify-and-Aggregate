...
from utils.memory import memory_sampling_balanced, generation_sampling
...
def main_worker(gpu, ngpus_per_node, config):
    ...
    if (task_step > 0) and (config['data_loader']['args']['memory']['mem_size'] > 0):
        ...
        if config['replay']['MaskGuide']['use_MaskGuide'] or config['replay']['TokenGuide']['use_TokenGuide']:
            print('maskguide', config['replay']['MaskGuide']['use_MaskGuide'])
            print('tokenguide', config['replay']['TokenGuide']['use_TokenGuide'])
            generation_sampling(
                config,
                model_old,
            ...
            if config['replay']['MaskGuide']['use_MaskGuide']:
                json_file_augX = []# json_file_augX
                if config['replay']['MaskGuide']['Replace']['use_Replace']:
                    if config['replay']['MaskGuide']['Replace']['times']>1:
                        for time in range(config['replay']['MaskGuide']['Replace']['times']):
                            json_file_augX.append(f'MaskGuide_{memory_size}_{time}X.json')
                    elif config['replay']['MaskGuide']['Replace']['times'] == 1:
                        json_file_augX = f'MaskGuide_{memory_size}.json'
                    else:
                        raise NotImplementedError
                    aug_ratio = config['replay']['MaskGuide']['Replace']['aug_ratio']
                
                elif config['replay']['MaskGuide']['Combine']['use_Combine']:
                    for time in range(config['replay']['MaskGuide']['Combine']['times']):
                        json_file_augX.append(f'MaskGuide_{memory_size}_{time}X.json')
                    aug_ratio = 1 #  we have a memory (ratio = 0) below + [augX] (ratio = 1)
                else:
                    raise NotImplementedError
                
                json_file_name = f'memory_{memory_size}.json'# json_file
                dataset.get_memory(config, json_file_name, json_file_augX, aug_ratio, concat=True)
            
            elif config['replay']['TokenGuide']['use_TokenGuide']:
                json_file_aug = f'TokenGuide_{memory_size}.json'
                json_file_name = f'memory_{memory_size}.json'
                aug_ratio = config['replay']['TokenGuide']['aug_ratio']
                dataset.get_memory(config, json_file_name, json_file_aug, aug_ratio, concat=True)
            else:
                raise NotImplementedError
        else:
            ...
            json_file_name = f'memory_{memory_size}.json'
            dataset.get_memory(config, json_file_name, concat=True)
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--not_use_MaskGuide'], action='store_false', target='replay;MaskGuide;use_MaskGuide'),
        CustomArgs(['--use_Replace'], action='store_true', target='replay;MaskGuide;Replace;use_Replace'),
        CustomArgs(['--use_Combine'], action='store_true', target='replay;MaskGuide;Combine;use_Combine'),
        ...
        CustomArgs(['--pos_weight_new'], type=float, target='hyperparameter;pos_weight_new'),
        CustomArgs(['--pos_weight_old'], type=float, target='hyperparameter;pos_weight_old'),
        CustomArgs(['--mbce'], type=float, target='hyperparameter;mbce'),
        CustomArgs(['--mbce_new_extra'], type=float, target='hyperparameter;mbce_new_extra'),
        CustomArgs(['--mbce_old_extra'], type=float, target='hyperparameter;mbce_old_extra'),
        CustomArgs(['--kd'], type=float, target='hyperparameter;kd'),
        CustomArgs(['--pkd'], type=float, target='hyperparameter;pkd'),
