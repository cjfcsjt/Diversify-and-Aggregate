...
class DeepLabV3(nn.Module):
    ...
    def forward(self, x, ret_intermediate=False, fake_features=None, region_bg=None):
        ...
        # grouding bg from features
        if region_bg is not None:
            extra_bg = x_pl.permute(1, 0, 2, 3)[:, region_bg].unsqueeze(0).unsqueeze(3)
            # add gaussian noise to extra_bg
            extra_bg = extra_bg + 0.1 * torch.randn_like(extra_bg).detach()
        ...
        if ret_intermediate:
            ...
            return sem_logits, {'neg_reg': sem_neg_logits_small, 'pos_reg': sem_pos_logits_small, 'features': features, 'logits_for_fake': logits_for_fake, 'logits_for_extra_bg': logits_for_extra_bg}
        else:
            return sem_logits, {'logits_for_fake': logits_for_fake, 'logits_for_extra_bg': logits_for_extra_bg}
